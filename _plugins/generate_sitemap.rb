...
#
# A generator that creates a sitemap.xml page for jekyll sites,
# suitable for submission to google etc.
#
# To use it, simply drop this script into the _plugins directory of
# your Jekyll site.
#
# When you compile your jekyll site, this plugin will loop through the
# list of pages in your site, and generate an entry in sitemap.xml for
# each one.
...
module Jekyll
  ...
  # Monkey-patch an accessor for a page's containing folder, since we
  # need it to generate the sitemap.
  class Page
    def subfolder
      @dir
    end
  ...
  # Sub-class Jekyll::StaticFile to allow recovery from unimportant
  # exception when writing the sitemap file.
  ...
  # Generates a sitemap.xml file containing URLs of all pages and
  # posts.
  class SitemapGenerator < Generator
    safe true
    priority :low
    
    # Domain that you are generating the sitemap for - update this to
    # match your site.
    BASE_URL = 'http://homodigitalis.delleside.org'
    ...
    def generate(site)
      ...
      # Add a static file entry for the zip file, otherwise
      # Site::cleanup will remove it.
    ...
    def generate_content(site)
      ...
      site.pages.each{ |page|
        if !page.pager
          subfolder = defined?(page.subfolder) ? page.subfolder + '/' : ''
          path     =  subfolder + page.name 
          mod_date =  File.mtime(site.source + '/' + path)
        elsif page.pager == true
          path = 'tag/' + page.subfolder + '/' + page.name
          mod_date = Time.now
        else
          path = 'page' + page.pager.page.to_s + '/' + page.name
          mod_date = Time.now
        end

        # Remove the trailing 'index.html' if there is one, and just
